...
function makeFilter(filterName: string, filterVal: number) {
    return `${filterName}(${filterVal})`;
}

export function makeFilterFunction(filterName: string, peakVal: number, baseVal: number) {
    return (t_eased: number) => {
        const filterVal = lerp(baseVal, peakVal, t_eased);
        const filter = makeFilter(filterName, filterVal);
